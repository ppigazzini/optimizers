...
def compute_c0_a0(
    n_steps: int, c_k_end: float, r_k_end: float, alpha: float, gamma: float
):
...
def compute_c1_a1(
    n_steps: int, c_k_end: float, r_k_end: float, alpha: float, gamma: float, A: float
):
...
def spsa_schedules(
    n_steps: int, a_0: float, A: float, alpha: float, c_0: float, gamma: float
):
...
def spsa_schedules(
    n_steps: int, a_1: float, A: float, alpha: float, c_1: float, gamma: float
):
    k = np.arange(1, n_steps + 1)
    a_k = a_1 / (k + A) ** alpha
    c_k = c_1 / k**gamma
...
if __name__ == "__main__":
    ...
    c1_1, a1_1 = compute_c1_a1(
        n_steps, c_k_end_1, r_k_end, DEFAULT_ALPHA, DEFAULT_GAMMA, A
    )
    c1_2, a1_2 = compute_c1_a1(
        n_steps, c_k_end_2, r_k_end, DEFAULT_ALPHA, DEFAULT_GAMMA, A
    )

    # Compute schedules
    k, a_k_1, c_k_1, r_k_1 = spsa_schedules(
        n_steps, a1_1, A, DEFAULT_ALPHA, c1_1, DEFAULT_GAMMA
    )
    _, a_k_2, c_k_2, r_k_2 = spsa_schedules(
        n_steps, a1_2, A, DEFAULT_ALPHA, c1_2, DEFAULT_GAMMA
    )

    # Compute difference in r_k
    r_k_diff = np.abs(r_k_2 - r_k_1)
    max_diff = np.max(r_k_diff)
    print(f"Max absolute difference between r_k sequences: {max_diff:.6g}")

    # Print r_1 and r_end for the plotted sequence
    r1_start = r_k_1[0]
    r1_end = r_k_1[-1]
    print(f"r_1 (start): {r1_start:.6g}")
    print(f"r_end (final): {r1_end:.6g}")
